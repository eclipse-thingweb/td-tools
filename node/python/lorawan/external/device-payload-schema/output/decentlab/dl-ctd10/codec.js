// TS013 Payload Codec — decentlab_dl_ctd10
// Schema version: 2
// Generated from: dl-ctd10.yaml
...
function decodePayload(buf, endian) {
  ...
  endian = endian || "big";
    ...
    // flagged on flags
    if (vars.flags & (1 << 0)) {
      var water_depth = readU(buf, pos, 2, endian);
      pos += 2;
      vars.water_depth = water_depth;
      d.water_depth = (water_depth + -32768);
      ...
      var freezing_flag = readU(buf, pos, 2, endian);
      pos += 2;
      vars.freezing_flag = freezing_flag;
      d.freezing_flag = freezing_flag;
    }
    if (vars.flags & (1 << 1)) {
...
function encodePayload(d, endian) {
  ...
  endian = endian || "big";
    ...
    // encode flagged
    var _flags = 0;
    if (d.water_depth !== undefined || d.temperature !== undefined || d.electrical_conductivity !== undefined || d.freezing_flag !== undefined) _flags |= (1 << 0);
    if (d.battery_voltage !== undefined) _flags |= (1 << 1);
    writeU(buf, pos - 2, 2, _flags, endian);
    if (_flags & (1 << 0)) {
      writeU(buf, pos, 2, Math.round(((d.water_depth !== undefined ? d.water_depth : 0) - (-32768))), endian);
      pos += 2;
      ...
      pos += 2;
      writeU(buf, pos, 2, Math.round((d.freezing_flag !== undefined ? d.freezing_flag : 0)), endian);
      pos += 2;
    }
    if (_flags & (1 << 1)) {
