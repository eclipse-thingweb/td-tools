// TS013 Payload Codec — decentlab_dl_lws
// Schema version: 2
// Generated from: dl-lws.yaml
...
function decodePayload(buf, endian) {
  ...
  endian = endian || "big";
    ...
    // flagged on flags
    if (vars.flags & (1 << 0)) {
      var leaf_wetness_index = readU(buf, pos, 2, endian);
      pos += 2;
      vars.leaf_wetness_index = leaf_wetness_index;
      d.leaf_wetness_index = leaf_wetness_index;
    }
    if (vars.flags & (1 << 1)) {
...
function encodePayload(d, endian) {
  ...
  endian = endian || "big";
    ...
    // encode flagged
    var _flags = 0;
    if (d.leaf_wetness_index !== undefined) _flags |= (1 << 0);
    if (d.battery_voltage !== undefined) _flags |= (1 << 1);
    writeU(buf, pos - 2, 2, _flags, endian);
    if (_flags & (1 << 0)) {
      writeU(buf, pos, 2, Math.round((d.leaf_wetness_index !== undefined ? d.leaf_wetness_index : 0)), endian);
      pos += 2;
    }
    if (_flags & (1 << 1)) {
