// TS013 Payload Codec — decentlab_dl_dlr2_002
// Schema version: 2
// Generated from: dl-dlr2-002.yaml
...
function decodePayload(buf, endian) {
  ...
  endian = endian || "big";
    ...
    // flagged on flags
    if (vars.flags & (1 << 0)) {
      var pulse_count = readU(buf, pos, 2, endian);
      pos += 2;
      vars.pulse_count = pulse_count;
      d.pulse_count = pulse_count;
      var pulse_interval = readU(buf, pos, 2, endian);
      pos += 2;
      vars.pulse_interval = pulse_interval;
      d.pulse_interval = pulse_interval;
      var cumulative_pulse_count = readU(buf, pos, 2, endian);
      pos += 2;
      vars.cumulative_pulse_count = cumulative_pulse_count;
      d.cumulative_pulse_count = cumulative_pulse_count;
    }
    if (vars.flags & (1 << 1)) {
...
function encodePayload(d, endian) {
  ...
  endian = endian || "big";
    ...
    // encode flagged
    var _flags = 0;
    if (d.pulse_count !== undefined || d.pulse_interval !== undefined || d.cumulative_pulse_count !== undefined) _flags |= (1 << 0);
    if (d.battery_voltage !== undefined) _flags |= (1 << 1);
    writeU(buf, pos - 2, 2, _flags, endian);
    if (_flags & (1 << 0)) {
      writeU(buf, pos, 2, Math.round((d.pulse_count !== undefined ? d.pulse_count : 0)), endian);
      pos += 2;
      writeU(buf, pos, 2, Math.round((d.pulse_interval !== undefined ? d.pulse_interval : 0)), endian);
      pos += 2;
      writeU(buf, pos, 2, Math.round((d.cumulative_pulse_count !== undefined ? d.cumulative_pulse_count : 0)), endian);
      pos += 2;
    }
    if (_flags & (1 << 1)) {
