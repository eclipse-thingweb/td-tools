// TS013 Payload Codec — decentlab_dl_isd
// Schema version: 2
// Generated from: dl-isd.yaml
...
function decodePayload(buf, endian) {
  ...
  endian = endian || "big";
    ...
    // flagged on flags
    if (vars.flags & (1 << 0)) {
      var stem_size = readU(buf, pos, 2, endian);
      pos += 2;
      vars.stem_size = stem_size;
      d.stem_size = (stem_size / 1000);
    }
    if (vars.flags & (1 << 1)) {
...
function encodePayload(d, endian) {
  ...
  endian = endian || "big";
    ...
    // encode flagged
    var _flags = 0;
    if (d.stem_size !== undefined) _flags |= (1 << 0);
    if (d.battery_voltage !== undefined) _flags |= (1 << 1);
    writeU(buf, pos - 2, 2, _flags, endian);
    if (_flags & (1 << 0)) {
      writeU(buf, pos, 2, Math.round(((d.stem_size !== undefined ? d.stem_size : 0) * 1000)), endian);
      pos += 2;
    }
    if (_flags & (1 << 1)) {
