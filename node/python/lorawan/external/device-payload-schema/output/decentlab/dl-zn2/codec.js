// TS013 Payload Codec — decentlab_dl_zn2
// Schema version: 2
// Generated from: dl-zn2.yaml
...
function decodePayload(buf, endian) {
  ...
  endian = endian || "big";
    ...
    // flagged on flags
    if (vars.flags & (1 << 0)) {
      var dendrometer_a_position = readU(buf, pos, 2, endian);
      pos += 2;
      vars.dendrometer_a_position = dendrometer_a_position;
      d.dendrometer_a_position = dendrometer_a_position;
      var dendrometer_b_position = readU(buf, pos, 2, endian);
      pos += 2;
      vars.dendrometer_b_position = dendrometer_b_position;
      d.dendrometer_b_position = dendrometer_b_position;
    }
    if (vars.flags & (1 << 1)) {
...
function encodePayload(d, endian) {
  ...
  endian = endian || "big";
    ...
    // encode flagged
    var _flags = 0;
    if (d.dendrometer_a_position !== undefined || d.dendrometer_b_position !== undefined) _flags |= (1 << 0);
    if (d.battery_voltage !== undefined) _flags |= (1 << 1);
    writeU(buf, pos - 2, 2, _flags, endian);
    if (_flags & (1 << 0)) {
      writeU(buf, pos, 2, Math.round((d.dendrometer_a_position !== undefined ? d.dendrometer_a_position : 0)), endian);
      pos += 2;
      writeU(buf, pos, 2, Math.round((d.dendrometer_b_position !== undefined ? d.dendrometer_b_position : 0)), endian);
      pos += 2;
    }
    if (_flags & (1 << 1)) {
