// TS013 Payload Codec — decentlab_dl_par
// Schema version: 2
// Generated from: dl-par.yaml
...
function decodePayload(buf, endian) {
  ...
  endian = endian || "big";
    ...
    // flagged on flags
    if (vars.flags & (1 << 0)) {
      var photosynthetically_active_radiation = readU(buf, pos, 2, endian);
      pos += 2;
      vars.photosynthetically_active_radiation = photosynthetically_active_radiation;
      d.photosynthetically_active_radiation = photosynthetically_active_radiation;
    }
    if (vars.flags & (1 << 1)) {
...
function encodePayload(d, endian) {
  ...
  endian = endian || "big";
    ...
    // encode flagged
    var _flags = 0;
    if (d.photosynthetically_active_radiation !== undefined) _flags |= (1 << 0);
    if (d.battery_voltage !== undefined) _flags |= (1 << 1);
    writeU(buf, pos - 2, 2, _flags, endian);
    if (_flags & (1 << 0)) {
      writeU(buf, pos, 2, Math.round((d.photosynthetically_active_radiation !== undefined ? d.photosynthetically_active_radiation : 0)), endian);
      pos += 2;
    }
    if (_flags & (1 << 1)) {
