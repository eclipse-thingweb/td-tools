// TS013 Payload Codec — decentlab_dl_zn1
// Schema version: 2
// Generated from: dl-zn1.yaml
...
function decodePayload(buf, endian) {
  ...
  endian = endian || "big";
    ...
    // flagged on flags
    if (vars.flags & (1 << 0)) {
      var dendrometer_position = readU(buf, pos, 2, endian);
      pos += 2;
      vars.dendrometer_position = dendrometer_position;
      d.dendrometer_position = dendrometer_position;
    }
    if (vars.flags & (1 << 1)) {
...
function encodePayload(d, endian) {
  ...
  endian = endian || "big";
    ...
    // encode flagged
    var _flags = 0;
    if (d.dendrometer_position !== undefined) _flags |= (1 << 0);
    if (d.battery_voltage !== undefined) _flags |= (1 << 1);
    writeU(buf, pos - 2, 2, _flags, endian);
    if (_flags & (1 << 0)) {
      writeU(buf, pos, 2, Math.round((d.dendrometer_position !== undefined ? d.dendrometer_position : 0)), endian);
      pos += 2;
    }
    if (_flags & (1 << 1)) {
