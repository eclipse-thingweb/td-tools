// TS013 Payload Codec — decentlab_dl_tbrg_01
// Schema version: 2
// Generated from: dl-tbrg-01.yaml
...
function decodePayload(buf, endian) {
  ...
  endian = endian || "big";
    ...
    if (vars.flags & (1 << 0)) {
      ...
      var precipitation_interval = readU(buf, pos, 2, endian);
      pos += 2;
      vars.precipitation_interval = precipitation_interval;
      d.precipitation_interval = precipitation_interval;
...
function encodePayload(d, endian) {
  ...
  endian = endian || "big";
    ...
    // encode flagged
    var _flags = 0;
    if (d.precipitation !== undefined || d.precipitation_interval !== undefined || d.cumulative_precipitation !== undefined) _flags |= (1 << 0);
    if (d.battery_voltage !== undefined) _flags |= (1 << 1);
    writeU(buf, pos - 2, 2, _flags, endian);
    if (_flags & (1 << 0)) {
      writeU(buf, pos, 2, Math.round((d.precipitation !== undefined ? d.precipitation : 0)), endian);
      pos += 2;
      writeU(buf, pos, 2, Math.round((d.precipitation_interval !== undefined ? d.precipitation_interval : 0)), endian);
      pos += 2;
