// TS013 Payload Codec — decentlab_dl_dlr2_012
// Schema version: 2
// Generated from: dl-dlr2-012.yaml
...
function decodePayload(buf, endian) {
  ...
  endian = endian || "big";
    ...
    // flagged on flags
    if (vars.flags & (1 << 0)) {
      var strain_gauge = readU(buf, pos, 2, endian);
      pos += 2;
      vars.strain_gauge = strain_gauge;
      d.strain_gauge = strain_gauge;
    }
    if (vars.flags & (1 << 1)) {
...
function encodePayload(d, endian) {
  ...
  endian = endian || "big";
    ...
    // encode flagged
    var _flags = 0;
    if (d.strain_gauge !== undefined) _flags |= (1 << 0);
    if (d.battery_voltage !== undefined) _flags |= (1 << 1);
    writeU(buf, pos - 2, 2, _flags, endian);
    if (_flags & (1 << 0)) {
      writeU(buf, pos, 2, Math.round((d.strain_gauge !== undefined ? d.strain_gauge : 0)), endian);
      pos += 2;
    }
    if (_flags & (1 << 1)) {
