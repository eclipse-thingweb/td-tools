// TS013 Payload Codec — decentlab_dl_pheht
// Schema version: 2
// Generated from: dl-pheht.yaml
...
function decodePayload(buf, endian) {
  ...
  endian = endian || "big";
    ...
    if (vars.flags & (1 << 0)) {
      ...
      var ph = readS(buf, pos, 2, endian);
      pos += 2;
      vars.ph = ph;
      d.ph = (ph / 100);
      var redox = readS(buf, pos, 2, endian);
      pos += 2;
      vars.redox = redox;
      d.redox = (redox / 10);
      var ph_mv = readS(buf, pos, 2, endian);
      pos += 2;
      vars.ph_mv = ph_mv;
      d.ph_mv = (ph_mv / 10);
    }
...
function encodePayload(d, endian) {
  ...
  endian = endian || "big";
    ...
    // encode flagged
    var _flags = 0;
    if (d.status !== undefined || d.temperature !== undefined || d.ph !== undefined || d.redox !== undefined || d.ph_mv !== undefined) _flags |= (1 << 0);
    ...
    if (_flags & (1 << 0)) {
      ...
      pos += 2;
      writeS(buf, pos, 2, Math.round(((d.ph !== undefined ? d.ph : 0) * 100)), endian);
      pos += 2;
      writeS(buf, pos, 2, Math.round(((d.redox !== undefined ? d.redox : 0) * 10)), endian);
      pos += 2;
      writeS(buf, pos, 2, Math.round(((d.ph_mv !== undefined ? d.ph_mv : 0) * 10)), endian);
      pos += 2;
    }
    if (_flags & (1 << 1)) {
