// TS013 Payload Codec — decentlab_dl_kl66_1538372_464859
// Schema version: 2
// Generated from: dl-kl66-1538372-464859.yaml
...
function decodePayload(buf, endian) {
  ...
  endian = endian || "big";
    ...
    // flagged on flags
    if (vars.flags & (1 << 0)) {
      var counter_reading = readU(buf, pos, 2, endian);
      pos += 2;
      vars.counter_reading = counter_reading;
      d.counter_reading = counter_reading;
      var measurement_interval = readU(buf, pos, 2, endian);
      pos += 2;
      vars.measurement_interval = measurement_interval;
      d.measurement_interval = (measurement_interval / 32768);
      var frequency = readU(buf, pos, 2, endian);
      pos += 2;
      vars.frequency = frequency;
      d.frequency = frequency;
      var weight = readU(buf, pos, 2, endian);
      pos += 2;
      vars.weight = weight;
      d.weight = weight;
      var elongation = readU(buf, pos, 2, endian);
      pos += 2;
      vars.elongation = elongation;
      d.elongation = elongation;
      var strain = readU(buf, pos, 2, endian);
      pos += 2;
      vars.strain = strain;
      d.strain = strain;
    }
...
function encodePayload(d, endian) {
  ...
  endian = endian || "big";
    ...
    // encode flagged
    var _flags = 0;
    if (d.counter_reading !== undefined || d.measurement_interval !== undefined || d.frequency !== undefined || d.weight !== undefined || d.elongation !== undefined || d.strain !== undefined) _flags |= (1 << 0);
    if (d.battery_voltage !== undefined) _flags |= (1 << 1);
    writeU(buf, pos - 2, 2, _flags, endian);
    if (_flags & (1 << 0)) {
      writeU(buf, pos, 2, Math.round((d.counter_reading !== undefined ? d.counter_reading : 0)), endian);
      pos += 2;
      writeU(buf, pos, 2, Math.round(((d.measurement_interval !== undefined ? d.measurement_interval : 0) * 32768)), endian);
      pos += 2;
      writeU(buf, pos, 2, Math.round((d.frequency !== undefined ? d.frequency : 0)), endian);
      pos += 2;
      writeU(buf, pos, 2, Math.round((d.weight !== undefined ? d.weight : 0)), endian);
      pos += 2;
      writeU(buf, pos, 2, Math.round((d.elongation !== undefined ? d.elongation : 0)), endian);
      pos += 2;
      writeU(buf, pos, 2, Math.round((d.strain !== undefined ? d.strain : 0)), endian);
      pos += 2;
    }
    if (_flags & (1 << 1)) {
