// TS013 Payload Codec — decentlab_dl_mes5
// Schema version: 2
// Generated from: dl-mes5.yaml
...
function decodePayload(buf, endian) {
  ...
  endian = endian || "big";
    ...
    if (vars.flags & (1 << 0)) {
      ...
      var sludge_blanket = readU(buf, pos, 2, endian);
      pos += 2;
      vars.sludge_blanket = sludge_blanket;
      d.sludge_blanket = (sludge_blanket / 100);
      var suspended_solid = readU(buf, pos, 2, endian);
      pos += 2;
      vars.suspended_solid = suspended_solid;
      d.suspended_solid = (suspended_solid / 100);
      var turbidity = readU(buf, pos, 2, endian);
      pos += 2;
      vars.turbidity = turbidity;
      d.turbidity = (turbidity / 10);
    }
    if (vars.flags & (1 << 1)) {
...
function encodePayload(d, endian) {
  ...
  endian = endian || "big";
    ...
    // encode flagged
    var _flags = 0;
    if (d.status !== undefined || d.temperature !== undefined || d.sludge_blanket !== undefined || d.suspended_solid !== undefined || d.turbidity !== undefined) _flags |= (1 << 0);
    ...
    if (_flags & (1 << 0)) {
      ...
      pos += 2;
      writeU(buf, pos, 2, Math.round(((d.sludge_blanket !== undefined ? d.sludge_blanket : 0) * 100)), endian);
      pos += 2;
      writeU(buf, pos, 2, Math.round(((d.suspended_solid !== undefined ? d.suspended_solid : 0) * 100)), endian);
      pos += 2;
      writeU(buf, pos, 2, Math.round(((d.turbidity !== undefined ? d.turbidity : 0) * 10)), endian);
      pos += 2;
    }
    if (_flags & (1 << 1)) {
