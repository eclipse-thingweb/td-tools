// TS013 Payload Codec — decentlab_dl_ntu
// Schema version: 2
// Generated from: dl-ntu.yaml
...
function decodePayload(buf, endian) {
  ...
  endian = endian || "big";
    ...
    if (vars.flags & (1 << 0)) {
      ...
      var turbidity_in_ntu = readU(buf, pos, 2, endian);
      pos += 2;
      vars.turbidity_in_ntu = turbidity_in_ntu;
      d.turbidity_in_ntu = (turbidity_in_ntu / 10);
      var turbidity_in_fnu = readU(buf, pos, 2, endian);
      pos += 2;
      vars.turbidity_in_fnu = turbidity_in_fnu;
      d.turbidity_in_fnu = (turbidity_in_fnu / 10);
      var turbidity_in_mg_l = readU(buf, pos, 2, endian);
      pos += 2;
      vars.turbidity_in_mg_l = turbidity_in_mg_l;
      d.turbidity_in_mg_l = (turbidity_in_mg_l / 10);
    }
    if (vars.flags & (1 << 1)) {
...
function encodePayload(d, endian) {
  ...
  endian = endian || "big";
    ...
    // encode flagged
    var _flags = 0;
    if (d.status !== undefined || d.temperature !== undefined || d.turbidity_in_ntu !== undefined || d.turbidity_in_fnu !== undefined || d.turbidity_in_mg_l !== undefined) _flags |= (1 << 0);
    ...
    if (_flags & (1 << 0)) {
      ...
      pos += 2;
      writeU(buf, pos, 2, Math.round(((d.turbidity_in_ntu !== undefined ? d.turbidity_in_ntu : 0) * 10)), endian);
      pos += 2;
      writeU(buf, pos, 2, Math.round(((d.turbidity_in_fnu !== undefined ? d.turbidity_in_fnu : 0) * 10)), endian);
      pos += 2;
      writeU(buf, pos, 2, Math.round(((d.turbidity_in_mg_l !== undefined ? d.turbidity_in_mg_l : 0) * 10)), endian);
      pos += 2;
    }
    if (_flags & (1 << 1)) {
