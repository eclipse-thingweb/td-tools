// TS013 Payload Codec — decentlab_dl_trs12
// Schema version: 2
// Generated from: dl-trs12.yaml
...
function decodePayload(buf, endian) {
  ...
  endian = endian || "big";
    ...
    if (vars.flags & (1 << 0)) {
      ...
      d.dielectric_permittivity = dielectric_permittivity;
      var volumetric_water_content = readU(buf, pos, 2, endian);
      pos += 2;
      vars.volumetric_water_content = volumetric_water_content;
      d.volumetric_water_content = volumetric_water_content;
      var soil_temperature = readS(buf, pos, 2, endian);
      pos += 2;
      vars.soil_temperature = soil_temperature;
      d.soil_temperature = (soil_temperature / 10);
      var electrical_conductivity = readU(buf, pos, 2, endian);
      pos += 2;
      vars.electrical_conductivity = electrical_conductivity;
      d.electrical_conductivity = electrical_conductivity;
    }
    if (vars.flags & (1 << 1)) {
...
function encodePayload(d, endian) {
  ...
  endian = endian || "big";
    ...
    // encode flagged
    var _flags = 0;
    if (d.dielectric_permittivity !== undefined || d.volumetric_water_content !== undefined || d.soil_temperature !== undefined || d.electrical_conductivity !== undefined) _flags |= (1 << 0);
    if (d.battery_voltage !== undefined) _flags |= (1 << 1);
    writeU(buf, pos - 2, 2, _flags, endian);
    if (_flags & (1 << 0)) {
      writeU(buf, pos, 2, Math.round((d.dielectric_permittivity !== undefined ? d.dielectric_permittivity : 0)), endian);
      pos += 2;
      writeU(buf, pos, 2, Math.round((d.volumetric_water_content !== undefined ? d.volumetric_water_content : 0)), endian);
      pos += 2;
      writeS(buf, pos, 2, Math.round(((d.soil_temperature !== undefined ? d.soil_temperature : 0) * 10)), endian);
      pos += 2;
      writeU(buf, pos, 2, Math.round((d.electrical_conductivity !== undefined ? d.electrical_conductivity : 0)), endian);
      pos += 2;
    }
    if (_flags & (1 << 1)) {
