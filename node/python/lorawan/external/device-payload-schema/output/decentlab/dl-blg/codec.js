// TS013 Payload Codec — decentlab_dl_blg
// Schema version: 2
// Generated from: dl-blg.yaml
...
function decodePayload(buf, endian) {
  ...
  endian = endian || "big";
    ...
    // flagged on flags
    if (vars.flags & (1 << 0)) {
      var voltage_ratio = readU(buf, pos, 2, endian);
      pos += 2;
      vars.voltage_ratio = voltage_ratio;
      d.voltage_ratio = voltage_ratio;
      var thermistor_resistance = readU(buf, pos, 2, endian);
      pos += 2;
      vars.thermistor_resistance = thermistor_resistance;
      d.thermistor_resistance = thermistor_resistance;
      var temperature = readU(buf, pos, 2, endian);
      pos += 2;
...
function encodePayload(d, endian) {
  ...
  endian = endian || "big";
    ...
    // encode flagged
    var _flags = 0;
    if (d.voltage_ratio !== undefined || d.thermistor_resistance !== undefined || d.temperature !== undefined) _flags |= (1 << 0);
    if (d.battery_voltage !== undefined) _flags |= (1 << 1);
    writeU(buf, pos - 2, 2, _flags, endian);
    if (_flags & (1 << 0)) {
      writeU(buf, pos, 2, Math.round((d.voltage_ratio !== undefined ? d.voltage_ratio : 0)), endian);
      pos += 2;
      writeU(buf, pos, 2, Math.round((d.thermistor_resistance !== undefined ? d.thermistor_resistance : 0)), endian);
      pos += 2;
      writeU(buf, pos, 2, Math.round((d.temperature !== undefined ? d.temperature : 0)), endian);
      pos += 2;
    }
    if (_flags & (1 << 1)) {
