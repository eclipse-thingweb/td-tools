// TS013 Payload Codec — decentlab_dl_mbx
// Schema version: 2
// Generated from: dl-mbx.yaml
...
function decodePayload(buf, endian) {
  ...
  endian = endian || "big";
    ...
    if (vars.flags & (1 << 0)) {
      ...
      d.distance = distance;
      var number_of_valid_samples = readU(buf, pos, 2, endian);
      pos += 2;
      vars.number_of_valid_samples = number_of_valid_samples;
      d.number_of_valid_samples = number_of_valid_samples;
    }
    if (vars.flags & (1 << 1)) {
...
function encodePayload(d, endian) {
  ...
  endian = endian || "big";
    ...
    // encode flagged
    var _flags = 0;
    if (d.distance !== undefined || d.number_of_valid_samples !== undefined) _flags |= (1 << 0);
    ...
    if (_flags & (1 << 0)) {
      ...
      pos += 2;
      writeU(buf, pos, 2, Math.round((d.number_of_valid_samples !== undefined ? d.number_of_valid_samples : 0)), endian);
      pos += 2;
    }
    if (_flags & (1 << 1)) {
