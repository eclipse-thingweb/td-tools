// TS013 Payload Codec — decentlab_dl_itst
// Schema version: 2
// Generated from: dl-itst.yaml
...
function decodePayload(buf, endian) {
  ...
  endian = endian || "big";
    ...
    // flagged on flags
    if (vars.flags & (1 << 0)) {
      var temperature_target = readU(buf, pos, 2, endian);
      pos += 2;
      vars.temperature_target = temperature_target;
      d.temperature_target = temperature_target;
      var temperature_head = readU(buf, pos, 2, endian);
      pos += 2;
      vars.temperature_head = temperature_head;
      d.temperature_head = temperature_head;
    }
    if (vars.flags & (1 << 1)) {
...
function encodePayload(d, endian) {
  ...
  endian = endian || "big";
    ...
    // encode flagged
    var _flags = 0;
    if (d.temperature_target !== undefined || d.temperature_head !== undefined) _flags |= (1 << 0);
    if (d.battery_voltage !== undefined) _flags |= (1 << 1);
    writeU(buf, pos - 2, 2, _flags, endian);
    if (_flags & (1 << 0)) {
      writeU(buf, pos, 2, Math.round((d.temperature_target !== undefined ? d.temperature_target : 0)), endian);
      pos += 2;
      writeU(buf, pos, 2, Math.round((d.temperature_head !== undefined ? d.temperature_head : 0)), endian);
      pos += 2;
    }
    if (_flags & (1 << 1)) {
