// TS013 Payload Codec — decentlab_dl_wrm
// Schema version: 2
// Generated from: dl-wrm.yaml
...
function decodePayload(buf, endian) {
  ...
  endian = endian || "big";
    ...
    if (vars.flags & (1 << 1)) {
      var surface_temperature = readU(buf, pos, 2, endian);
      pos += 2;
      vars.surface_temperature = surface_temperature;
      d.surface_temperature = surface_temperature;
      var head_temperature = readU(buf, pos, 2, endian);
      pos += 2;
      vars.head_temperature = head_temperature;
      d.head_temperature = head_temperature;
    }
    if (vars.flags & (1 << 2)) {
...
function encodePayload(d, endian) {
  ...
  endian = endian || "big";
    ...
    if (d.surface_temperature !== undefined || d.head_temperature !== undefined) _flags |= (1 << 1);
    ...
    if (_flags & (1 << 1)) {
      writeU(buf, pos, 2, Math.round((d.surface_temperature !== undefined ? d.surface_temperature : 0)), endian);
      pos += 2;
      writeU(buf, pos, 2, Math.round((d.head_temperature !== undefined ? d.head_temperature : 0)), endian);
      pos += 2;
    }
    if (_flags & (1 << 2)) {
