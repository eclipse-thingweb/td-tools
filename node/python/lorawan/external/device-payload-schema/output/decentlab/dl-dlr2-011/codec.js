// TS013 Payload Codec — decentlab_dl_dlr2_011
// Schema version: 2
// Generated from: dl-dlr2-011.yaml
...
function decodePayload(buf, endian) {
  ...
  endian = endian || "big";
    ...
    // flagged on flags
    if (vars.flags & (1 << 0)) {
      var ch0_input = readU(buf, pos, 2, endian);
      pos += 2;
      vars.ch0_input = ch0_input;
      d.ch0_input = ch0_input;
    }
    if (vars.flags & (1 << 1)) {
      var ch1_input = readU(buf, pos, 2, endian);
      pos += 2;
      vars.ch1_input = ch1_input;
      d.ch1_input = ch1_input;
    }
    if (vars.flags & (1 << 2)) {
...
function encodePayload(d, endian) {
  ...
  endian = endian || "big";
    ...
    // encode flagged
    var _flags = 0;
    if (d.ch0_input !== undefined) _flags |= (1 << 0);
    if (d.ch1_input !== undefined) _flags |= (1 << 1);
    if (d.battery_voltage !== undefined) _flags |= (1 << 2);
    writeU(buf, pos - 2, 2, _flags, endian);
    if (_flags & (1 << 0)) {
      writeU(buf, pos, 2, Math.round((d.ch0_input !== undefined ? d.ch0_input : 0)), endian);
      pos += 2;
    }
    if (_flags & (1 << 1)) {
      writeU(buf, pos, 2, Math.round((d.ch1_input !== undefined ? d.ch1_input : 0)), endian);
      pos += 2;
    }
    if (_flags & (1 << 2)) {
