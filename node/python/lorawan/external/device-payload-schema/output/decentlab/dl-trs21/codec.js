// TS013 Payload Codec — decentlab_dl_trs21
// Schema version: 2
// Generated from: dl-trs21.yaml
...
function decodePayload(buf, endian) {
  ...
  endian = endian || "big";
    ...
    // flagged on flags
    if (vars.flags & (1 << 0)) {
      var water_potential = readU(buf, pos, 2, endian);
      pos += 2;
      vars.water_potential = water_potential;
      d.water_potential = water_potential;
...
function encodePayload(d, endian) {
  ...
  endian = endian || "big";
    ...
    // encode flagged
    var _flags = 0;
    if (d.water_potential !== undefined || d.soil_temperature !== undefined) _flags |= (1 << 0);
    if (d.battery_voltage !== undefined) _flags |= (1 << 1);
    writeU(buf, pos - 2, 2, _flags, endian);
    if (_flags & (1 << 0)) {
      writeU(buf, pos, 2, Math.round((d.water_potential !== undefined ? d.water_potential : 0)), endian);
      pos += 2;
