// TS013 Payload Codec — decentlab_dl_dlr2_003
// Schema version: 2
// Generated from: dl-dlr2-003.yaml
...
function decodePayload(buf, endian) {
  ...
  endian = endian || "big";
    ...
    // flagged on flags
    if (vars.flags & (1 << 0)) {
      var input = readU(buf, pos, 2, endian);
      pos += 2;
      vars.input = input;
      d.input = input;
    }
...
function encodePayload(d, endian) {
  ...
  endian = endian || "big";
    ...
    // encode flagged
    var _flags = 0;
    if (d.input !== undefined) _flags |= (1 << 0);
    if (d.battery_voltage !== undefined) _flags |= (1 << 1);
    writeU(buf, pos - 2, 2, _flags, endian);
    if (_flags & (1 << 0)) {
      writeU(buf, pos, 2, Math.round((d.input !== undefined ? d.input : 0)), endian);
      pos += 2;
    }
    if (_flags & (1 << 1)) {
