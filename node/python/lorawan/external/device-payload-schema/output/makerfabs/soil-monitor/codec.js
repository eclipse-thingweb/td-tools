// TS013 Payload Codec — makerfabs_soil_monitor
// Schema version: 1
// Generated from: soil-monitor.yaml
...
function decodePayload(buf, endian) {
  var pos = 0, d = {}, vars = {};
  endian = endian || "big";
    var _num = readU(buf, pos, 2, endian);
    pos += 2;
    vars._num = _num;
    ...
    var _significant = readU(buf, pos, 1, endian);
    pos += 1;
    vars._significant = _significant;
    var humidity = readU(buf, pos, 2, endian);
    pos += 2;
    vars.humidity = humidity;
    d.humidity = (humidity / 10);
    var temperature = readS(buf, pos, 2, endian);
    pos += 2;
    vars.temperature = temperature;
    d.temperature = (temperature / 10);
    var ec = readU(buf, pos, 2, endian);
    pos += 2;
    vars.ec = ec;
    d.ec = ec;
    var ph = readU(buf, pos, 2, endian);
    pos += 2;
    vars.ph = ph;
    d.ph = (ph / 10);
    var interval = readU(buf, pos, 4, endian);
    pos += 4;
    vars.interval = interval;
    d.interval = (interval / 1000);
...
function encodePayload(d, endian) {
  ...
  var pos = 0;
  endian = endian || "big";
    writeU(buf, pos, 2, 0, endian);
    pos += 2;
    writeU(buf, pos, 1, Math.round(((d.battery !== undefined ? d.battery : 0) * 10)), endian);
    pos += 1;
    writeU(buf, pos, 1, 0, endian);
    pos += 1;
    writeU(buf, pos, 2, Math.round(((d.humidity !== undefined ? d.humidity : 0) * 10)), endian);
    pos += 2;
    writeS(buf, pos, 2, Math.round(((d.temperature !== undefined ? d.temperature : 0) * 10)), endian);
    pos += 2;
    writeU(buf, pos, 2, Math.round((d.ec !== undefined ? d.ec : 0)), endian);
    pos += 2;
    writeU(buf, pos, 2, Math.round(((d.ph !== undefined ? d.ph : 0) * 10)), endian);
    pos += 2;
    writeU(buf, pos, 4, Math.round(((d.interval !== undefined ? d.interval : 0) * 1000)), endian);
    pos += 4;
