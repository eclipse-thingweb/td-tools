// TS013 Payload Codec — milesight_ws201
// Schema version: 1
// Generated from: ws201.yaml
  ...
  endian = endian || "little";
    ...
    while (pos + 2 <= buf.length) {
      ...
      if ("[" + channel_id + ", " + channel_type + "]" === "[1, 117]") {
        ...
        d.battery = battery;
      } else if ("[" + channel_id + ", " + channel_type + "]" === "[3, 130]") {
        var distance = readU(buf, pos, 2, endian);
        pos += 2;
        vars.distance = distance;
        d.distance = distance;
      } else if ("[" + channel_id + ", " + channel_type + "]" === "[4, 214]") {
        var remaining = readU(buf, pos, 1, endian);
        pos += 1;
...
function encodePayload(d, endian) {
  ...
  endian = endian || "little";
    ...
    if (d.remaining !== undefined) {
      ...
      pos += 1;
      writeU(buf, pos, 1, 214, endian);
      pos += 1;
      writeU(buf, pos, 1, Math.round((d.remaining !== undefined ? d.remaining : 0)), endian);
      pos += 1;
    }
