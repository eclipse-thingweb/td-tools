// TS013 Payload Codec — milesight_gs301
// Schema version: 1
// Generated from: gs301.yaml
  ...
  endian = endian || "little";
    ...
    while (pos + 2 <= buf.length) {
      ...
      if ("[" + channel_id + ", " + channel_type + "]" === "[255, 1]") {
        ...
        d.battery = battery;
      } else if ("[" + channel_id + ", " + channel_type + "]" === "[2, 103]") {
        var temperature = readS(buf, pos, 2, endian);
        pos += 2;
        vars.temperature = temperature;
        d.temperature = (temperature / 10);
      } else if ("[" + channel_id + ", " + channel_type + "]" === "[3, 104]") {
        var humidity = readU(buf, pos, 1, endian);
        pos += 1;
        vars.humidity = humidity;
        d.humidity = humidity;
      } else if ("[" + channel_id + ", " + channel_type + "]" === "[4, 125]") {
        var nh3_sensor_status = readU(buf, pos, 1, endian);
        pos += 1;
        vars.nh3_sensor_status = nh3_sensor_status;
        d.nh3_sensor_status = nh3_sensor_status;
        var nh3 = readU(buf, pos, 1, endian);
        pos += 1;
        vars.nh3 = nh3;
        d.nh3 = nh3;
      } else if ("[" + channel_id + ", " + channel_type + "]" === "[5, 125]") {
        var h2s_sensor_status = readU(buf, pos, 1, endian);
        pos += 1;
        vars.h2s_sensor_status = h2s_sensor_status;
        d.h2s_sensor_status = h2s_sensor_status;
        var h2s = readU(buf, pos, 1, endian);
        pos += 1;
        vars.h2s = h2s;
        d.h2s = h2s;
      } else if ("[" + channel_id + ", " + channel_type + "]" === "[6, 125]") {
        var h2s_sensor_status = readU(buf, pos, 1, endian);
        pos += 1;
        vars.h2s_sensor_status = h2s_sensor_status;
        d.h2s_sensor_status = h2s_sensor_status;
        ...
        vars.h2s = h2s;
        d.h2s = h2s;
      } else if ("[" + channel_id + ", " + channel_type + "]" === "[7, 234]") {
        var nh3_calibration_result = readU(buf, pos, 1, endian);
        pos += 1;
        vars.nh3_calibration_result = nh3_calibration_result;
        d.nh3_calibration_result = nh3_calibration_result;
      } else if ("[" + channel_id + ", " + channel_type + "]" === "[255, 124]") {
        var sensor_id = readU(buf, pos, 1, endian);
        pos += 1;
        vars.sensor_id = sensor_id;
...
function encodePayload(d, endian) {
  ...
  endian = endian || "little";
    ...
    if (d.nh3_sensor_status !== undefined || d.nh3 !== undefined) {
      writeU(buf, pos, 1, 4, endian);
      pos += 1;
      writeU(buf, pos, 1, 125, endian);
      pos += 1;
      writeU(buf, pos, 1, Math.round((d.nh3_sensor_status !== undefined ? d.nh3_sensor_status : 0)), endian);
      pos += 1;
      writeU(buf, pos, 1, Math.round((d.nh3 !== undefined ? d.nh3 : 0)), endian);
      pos += 1;
    }
    if (d.h2s_sensor_status !== undefined || d.h2s !== undefined) {
      writeU(buf, pos, 1, 5, endian);
      pos += 1;
      writeU(buf, pos, 1, 125, endian);
      pos += 1;
      writeU(buf, pos, 1, Math.round((d.h2s_sensor_status !== undefined ? d.h2s_sensor_status : 0)), endian);
      pos += 1;
      writeU(buf, pos, 1, Math.round((d.h2s !== undefined ? d.h2s : 0)), endian);
      pos += 1;
    }
    if (d.h2s_sensor_status !== undefined || d.h2s !== undefined) {
    ...
    if (d.nh3_calibration_result !== undefined) {
      writeU(buf, pos, 1, 7, endian);
      pos += 1;
      writeU(buf, pos, 1, 234, endian);
      pos += 1;
      writeU(buf, pos, 1, Math.round((d.nh3_calibration_result !== undefined ? d.nh3_calibration_result : 0)), endian);
      pos += 1;
    }
    if (d.sensor_id !== undefined) {
      writeU(buf, pos, 1, 255, endian);
      pos += 1;
      writeU(buf, pos, 1, 124, endian);
      pos += 1;
      writeU(buf, pos, 1, Math.round((d.sensor_id !== undefined ? d.sensor_id : 0)), endian);
      pos += 1;
    }
