// TS013 Payload Codec — milesight_em300_mld
// Schema version: 1
// Generated from: em300-mld.yaml
  ...
  endian = endian || "little";
    ...
    while (pos + 2 <= buf.length) {
      ...
      if ("[" + channel_id + ", " + channel_type + "]" === "[1, 117]") {
        ...
        var leakage_status = readU(buf, pos, 1, endian);
        pos += 1;
        vars.leakage_status = leakage_status;
        d.leakage_status = leakage_status;
      } else {
        break;
      }
    }
...
function encodePayload(d, endian) {
  ...
  endian = endian || "little";
    ...
    if (d.leakage_status !== undefined) {
      writeU(buf, pos, 1, 5, endian);
      pos += 1;
      writeU(buf, pos, 1, 0, endian);
      pos += 1;
      writeU(buf, pos, 1, Math.round((d.leakage_status !== undefined ? d.leakage_status : 0)), endian);
      pos += 1;
    }
