// TS013 Payload Codec — milesight_ct105
// Schema version: 1
// Generated from: ct105.yaml
...
function decodePayload(buf, endian) {
  ...
  endian = endian || "little";
    ...
    while (pos + 2 <= buf.length) {
      ...
      } else if ("[" + channel_id + ", " + channel_type + "]" === "[3, 151]") {
        var total_current = readU(buf, pos, 4, endian);
        pos += 4;
        vars.total_current = total_current;
        d.total_current = (total_current / 100);
      } else if ("[" + channel_id + ", " + channel_type + "]" === "[4, 152]") {
        var current_sensor_status = readU(buf, pos, 1, endian);
        pos += 1;
        vars.current_sensor_status = current_sensor_status;
        d.current_sensor_status = current_sensor_status;
      ...
      } else if ("[" + channel_id + ", " + channel_type + "]" === "[132, 152]") {
        var current_max = readU(buf, pos, 2, endian);
        pos += 2;
        vars.current_max = current_max;
        d.current_max = (current_max / 100);
        var current_min = readU(buf, pos, 2, endian);
        pos += 2;
        vars.current_min = current_min;
        d.current_min = (current_min / 100);
        var current = readU(buf, pos, 2, endian);
        pos += 2;
        vars.current = current;
        d.current = (current / 100);
        var current_alarm = readU(buf, pos, 1, endian);
        pos += 1;
        vars.current_alarm = current_alarm;
        d.current_alarm = current_alarm;
...
function encodePayload(d, endian) {
  ...
  endian = endian || "little";
    ...
    if (d.total_current !== undefined) {
      ...
      pos += 1;
      writeU(buf, pos, 1, 151, endian);
      pos += 1;
      writeU(buf, pos, 4, Math.round(((d.total_current !== undefined ? d.total_current : 0) * 100)), endian);
      pos += 4;
    }
    if (d.current_sensor_status !== undefined) {
      writeU(buf, pos, 1, 4, endian);
      pos += 1;
      writeU(buf, pos, 1, 152, endian);
      pos += 1;
      writeU(buf, pos, 1, Math.round((d.current_sensor_status !== undefined ? d.current_sensor_status : 0)), endian);
      pos += 1;
    }
    ...
    if (d.current_max !== undefined || d.current_min !== undefined || d.current !== undefined || d.current_alarm !== undefined) {
      writeU(buf, pos, 1, 132, endian);
      pos += 1;
      writeU(buf, pos, 1, 152, endian);
      pos += 1;
      writeU(buf, pos, 2, Math.round(((d.current_max !== undefined ? d.current_max : 0) * 100)), endian);
      pos += 2;
      writeU(buf, pos, 2, Math.round(((d.current_min !== undefined ? d.current_min : 0) * 100)), endian);
      pos += 2;
      writeU(buf, pos, 2, Math.round(((d.current !== undefined ? d.current : 0) * 100)), endian);
      pos += 2;
      writeU(buf, pos, 1, Math.round((d.current_alarm !== undefined ? d.current_alarm : 0)), endian);
      pos += 1;
    }
