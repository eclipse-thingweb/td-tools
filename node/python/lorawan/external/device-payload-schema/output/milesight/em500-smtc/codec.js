// TS013 Payload Codec — milesight_em500_smtc
// Schema version: 1
// Generated from: em500-smtc.yaml
...
function decodePayload(buf, endian) {
  ...
  endian = endian || "little";
    ...
    while (pos + 2 <= buf.length) {
      ...
      } else if ("[" + channel_id + ", " + channel_type + "]" === "[4, 202]") {
        var humidity = readU(buf, pos, 1, endian);
        pos += 1;
        vars.humidity = humidity;
        d.humidity = (humidity / 2);
      } else if ("[" + channel_id + ", " + channel_type + "]" === "[5, 127]") {
        var conductivity = readU(buf, pos, 2, endian);
        pos += 2;
        vars.conductivity = conductivity;
        d.conductivity = conductivity;
      } else {
        break;
      }
    }
...
function encodePayload(d, endian) {
  ...
  endian = endian || "little";
    ...
    if (d.humidity !== undefined) {
      writeU(buf, pos, 1, 4, endian);
      pos += 1;
      writeU(buf, pos, 1, 202, endian);
      pos += 1;
      writeU(buf, pos, 1, Math.round(((d.humidity !== undefined ? d.humidity : 0) * 2)), endian);
      pos += 1;
    }
    if (d.conductivity !== undefined) {
      writeU(buf, pos, 1, 5, endian);
      pos += 1;
      writeU(buf, pos, 1, 127, endian);
      pos += 1;
      writeU(buf, pos, 2, Math.round((d.conductivity !== undefined ? d.conductivity : 0)), endian);
      pos += 2;
    }
