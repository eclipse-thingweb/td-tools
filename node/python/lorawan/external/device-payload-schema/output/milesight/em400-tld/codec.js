// TS013 Payload Codec — milesight_em400_tld
// Schema version: 1
// Generated from: em400-tld.yaml
...
function decodePayload(buf, endian) {
  ...
  endian = endian || "little";
    ...
    while (pos + 2 <= buf.length) {
      ...
      if ("[" + channel_id + ", " + channel_type + "]" === "[1, 117]") {
      ...
      } else if ("[" + channel_id + ", " + channel_type + "]" === "[4, 130]") {
        ...
        vars.temperature = temperature;
        d.temperature = (temperature / 10);
        var temperature_abnormal = readU(buf, pos, 1, endian);
        pos += 1;
        vars.temperature_abnormal = temperature_abnormal;
        d.temperature_abnormal = temperature_abnormal;
      } else if ("[" + channel_id + ", " + channel_type + "]" === "[132, 130]") {
        var distance = readU(buf, pos, 2, endian);
        pos += 2;
        vars.distance = distance;
        d.distance = distance;
        var distance_alarming = readU(buf, pos, 1, endian);
        pos += 1;
        vars.distance_alarming = distance_alarming;
        d.distance_alarming = distance_alarming;
      } else {
        break;
      }
    }
...
function encodePayload(d, endian) {
  ...
  endian = endian || "little";
    ...
    if (d.temperature !== undefined || d.temperature_abnormal !== undefined) {
      ...
      writeS(buf, pos, 2, Math.round(((d.temperature !== undefined ? d.temperature : 0) * 10)), endian);
      pos += 2;
      writeU(buf, pos, 1, Math.round((d.temperature_abnormal !== undefined ? d.temperature_abnormal : 0)), endian);
      pos += 1;
    }
    if (d.distance !== undefined || d.distance_alarming !== undefined) {
      ...
      writeU(buf, pos, 2, Math.round((d.distance !== undefined ? d.distance : 0)), endian);
      pos += 2;
      writeU(buf, pos, 1, Math.round((d.distance_alarming !== undefined ? d.distance_alarming : 0)), endian);
      pos += 1;
    }
