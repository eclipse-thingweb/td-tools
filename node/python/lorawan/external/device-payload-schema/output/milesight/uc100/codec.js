// TS013 Payload Codec — milesight_uc100
// Schema version: 1
// Generated from: uc100.yaml
...
function decodePayload(buf, endian) {
  ...
  endian = endian || "little";
    ...
    while (pos + 2 <= buf.length) {
      ...
      pos += 1;
      if ("[" + channel_id + ", " + channel_type + "]" === "[255, 11]") {
        var power = readU(buf, pos, 1, endian);
        pos += 1;
        vars.power = power;
        d.power = power;
      } else if ("[" + channel_id + ", " + channel_type + "]" === "[255, 1]") {
        ...
        var software_version = readU(buf, pos, 1, endian);
        pos += 1;
        vars.software_version = software_version;
...
function encodePayload(d, endian) {
  ...
  endian = endian || "little";
    ...
    if (d.power !== undefined) {
      ...
      pos += 1;
      writeU(buf, pos, 1, Math.round((d.power !== undefined ? d.power : 0)), endian);
      pos += 1;
    }
    ...
    if (d.software_version !== undefined) {
      writeU(buf, pos, 1, 255, endian);
      pos += 1;
      writeU(buf, pos, 1, 10, endian);
      pos += 1;
      writeU(buf, pos, 1, Math.round((d.software_version !== undefined ? d.software_version : 0)), endian);
      pos += 1;
    }
