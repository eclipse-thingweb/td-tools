// TS013 Payload Codec — milesight_ws301
// Schema version: 1
// Generated from: ws301.yaml
  ...
  endian = endian || "little";
    ...
    while (pos + 2 <= buf.length) {
      ...
      if ("[" + channel_id + ", " + channel_type + "]" === "[1, 117]") {
        ...
        var door = readU(buf, pos, 1, endian);
        pos += 1;
        vars.door = door;
        d.door = door;
      } else if ("[" + channel_id + ", " + channel_type + "]" === "[4, 0]") {
        var install = readU(buf, pos, 1, endian);
        pos += 1;
...
function encodePayload(d, endian) {
  ...
  endian = endian || "little";
    ...
    if (d.door !== undefined) {
      ...
      pos += 1;
      writeU(buf, pos, 1, Math.round((d.door !== undefined ? d.door : 0)), endian);
      pos += 1;
    }
    if (d.install !== undefined) {
      ...
      writeU(buf, pos, 1, 0, endian);
      pos += 1;
      writeU(buf, pos, 1, Math.round((d.install !== undefined ? d.install : 0)), endian);
      pos += 1;
    }
