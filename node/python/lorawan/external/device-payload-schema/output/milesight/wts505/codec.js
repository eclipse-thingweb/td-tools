// TS013 Payload Codec — milesight_wts505
// Schema version: 1
// Generated from: wts505.yaml
...
function decodePayload(buf, endian) {
  ...
  endian = endian || "little";
    ...
    while (pos + 2 <= buf.length) {
      ...
      } else if ("[" + channel_id + ", " + channel_type + "]" === "[5, 132]") {
        var wind_direction = readS(buf, pos, 2, endian);
        pos += 2;
        vars.wind_direction = wind_direction;
        d.wind_direction = (wind_direction / 10);
      } else if ("[" + channel_id + ", " + channel_type + "]" === "[6, 115]") {
        var pressure = readU(buf, pos, 2, endian);
        pos += 2;
        vars.pressure = pressure;
        d.pressure = (pressure / 10);
      } else if ("[" + channel_id + ", " + channel_type + "]" === "[7, 146]") {
        var wind_speed = readU(buf, pos, 2, endian);
        pos += 2;
        vars.wind_speed = wind_speed;
        d.wind_speed = (wind_speed / 10);
      } else if ("[" + channel_id + ", " + channel_type + "]" === "[8, 119]") {
        var rainfall_total = readU(buf, pos, 2, endian);
        pos += 2;
        vars.rainfall_total = rainfall_total;
        d.rainfall_total = (rainfall_total / 100);
        var rainfall_counter = readU(buf, pos, 1, endian);
        pos += 1;
        vars.rainfall_counter = rainfall_counter;
        d.rainfall_counter = rainfall_counter;
      } else {
        break;
      }
    }
...
function encodePayload(d, endian) {
  ...
  endian = endian || "little";
    ...
    if (d.wind_speed !== undefined) {
      ...
      pos += 1;
      writeU(buf, pos, 2, Math.round(((d.wind_speed !== undefined ? d.wind_speed : 0) * 10)), endian);
      pos += 2;
    }
    if (d.rainfall_total !== undefined || d.rainfall_counter !== undefined) {
      writeU(buf, pos, 1, 8, endian);
      pos += 1;
      writeU(buf, pos, 1, 119, endian);
      pos += 1;
      writeU(buf, pos, 2, Math.round(((d.rainfall_total !== undefined ? d.rainfall_total : 0) * 100)), endian);
      pos += 2;
      writeU(buf, pos, 1, Math.round((d.rainfall_counter !== undefined ? d.rainfall_counter : 0)), endian);
      pos += 1;
    }
