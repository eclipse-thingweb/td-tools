// TS013 Payload Codec — milesight_am103l
// Schema version: 1
// Generated from: am103l.yaml
...
function decodePayload(buf, endian) {
  ...
  endian = endian || "little";
    ...
    while (pos + 2 <= buf.length) {
      ...
      } else if ("[" + channel_id + ", " + channel_type + "]" === "[5, 106]") {
        var activity = readU(buf, pos, 2, endian);
        pos += 2;
        vars.activity = activity;
        d.activity = activity;
      } else if ("[" + channel_id + ", " + channel_type + "]" === "[6, 101]") {
        var illumination = readU(buf, pos, 2, endian);
        pos += 2;
        vars.illumination = illumination;
        d.illumination = illumination;
        var infrared_and_visible = readU(buf, pos, 2, endian);
        pos += 2;
        vars.infrared_and_visible = infrared_and_visible;
        d.infrared_and_visible = infrared_and_visible;
        var infrared = readU(buf, pos, 2, endian);
        pos += 2;
        vars.infrared = infrared;
        d.infrared = infrared;
...
function encodePayload(d, endian) {
  ...
  endian = endian || "little";
    ...
    if (d.activity !== undefined) {
      ...
      pos += 1;
      writeU(buf, pos, 1, 106, endian);
      pos += 1;
      writeU(buf, pos, 2, Math.round((d.activity !== undefined ? d.activity : 0)), endian);
      pos += 2;
    }
    if (d.illumination !== undefined || d.infrared_and_visible !== undefined || d.infrared !== undefined) {
      writeU(buf, pos, 1, 6, endian);
      pos += 1;
      writeU(buf, pos, 1, 101, endian);
      pos += 1;
      writeU(buf, pos, 2, Math.round((d.illumination !== undefined ? d.illumination : 0)), endian);
      pos += 2;
      writeU(buf, pos, 2, Math.round((d.infrared_and_visible !== undefined ? d.infrared_and_visible : 0)), endian);
      pos += 2;
      writeU(buf, pos, 2, Math.round((d.infrared !== undefined ? d.infrared : 0)), endian);
      pos += 2;
    }
    if (d.co2 !== undefined) {
