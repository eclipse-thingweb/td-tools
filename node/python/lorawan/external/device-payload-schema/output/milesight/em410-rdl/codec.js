// TS013 Payload Codec — milesight_em410_rdl
// Schema version: 1
// Generated from: em410-rdl.yaml
  ...
  endian = endian || "little";
    ...
    while (pos + 2 <= buf.length) {
      ...
      if ("[" + channel_id + ", " + channel_type + "]" === "[255, 1]") {
        ...
        var distance = readS(buf, pos, 2, endian);
        pos += 2;
        ...
        d.position = position;
      } else if ("[" + channel_id + ", " + channel_type + "]" === "[6, 199]") {
        var radar_signal_rssi = readS(buf, pos, 2, endian);
        pos += 2;
        vars.radar_signal_rssi = radar_signal_rssi;
        d.radar_signal_rssi = (radar_signal_rssi / 100);
      } else {
        break;
      }
    }
...
function encodePayload(d, endian) {
  ...
  endian = endian || "little";
    ...
    if (d.distance !== undefined) {
      ...
      pos += 1;
      writeS(buf, pos, 2, Math.round((d.distance !== undefined ? d.distance : 0)), endian);
      pos += 2;
    }
    ...
    if (d.radar_signal_rssi !== undefined) {
      writeU(buf, pos, 1, 6, endian);
      pos += 1;
      writeU(buf, pos, 1, 199, endian);
      pos += 1;
      writeS(buf, pos, 2, Math.round(((d.radar_signal_rssi !== undefined ? d.radar_signal_rssi : 0) * 100)), endian);
      pos += 2;
    }
