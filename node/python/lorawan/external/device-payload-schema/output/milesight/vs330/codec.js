// TS013 Payload Codec — milesight_vs330
// Schema version: 1
// Generated from: vs330.yaml
  ...
  endian = endian || "little";
    ...
    while (pos + 2 <= buf.length) {
      ...
      if ("[" + channel_id + ", " + channel_type + "]" === "[1, 117]") {
        ...
        d.battery = battery;
      } else if ("[" + channel_id + ", " + channel_type + "]" === "[2, 130]") {
        var distance = readU(buf, pos, 2, endian);
        pos += 2;
        vars.distance = distance;
        d.distance = distance;
      } else if ("[" + channel_id + ", " + channel_type + "]" === "[3, 142]") {
        var occupancy = readU(buf, pos, 1, endian);
        pos += 1;
        vars.occupancy = occupancy;
        d.occupancy = occupancy;
      } else if ("[" + channel_id + ", " + channel_type + "]" === "[4, 142]") {
        var calibration = readU(buf, pos, 1, endian);
        pos += 1;
        vars.calibration = calibration;
...
function encodePayload(d, endian) {
  ...
  endian = endian || "little";
    ...
    if (d.calibration !== undefined) {
      writeU(buf, pos, 1, 4, endian);
      pos += 1;
      writeU(buf, pos, 1, 142, endian);
      pos += 1;
      writeU(buf, pos, 1, Math.round((d.calibration !== undefined ? d.calibration : 0)), endian);
      pos += 1;
    }
