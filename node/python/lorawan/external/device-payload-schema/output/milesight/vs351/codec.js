// TS013 Payload Codec — milesight_vs351
// Schema version: 1
// Generated from: vs351.yaml
...
function decodePayload(buf, endian) {
  ...
  endian = endian || "little";
    ...
    while (pos + 2 <= buf.length) {
      ...
      } else if ("[" + channel_id + ", " + channel_type + "]" === "[255, 204]") {
        var power_status = readU(buf, pos, 1, endian);
        pos += 1;
        vars.power_status = power_status;
        d.power_status = power_status;
...
function encodePayload(d, endian) {
  ...
  endian = endian || "little";
    ...
    if (d.power_status !== undefined) {
      writeU(buf, pos, 1, 255, endian);
      pos += 1;
      writeU(buf, pos, 1, 204, endian);
      pos += 1;
      writeU(buf, pos, 1, Math.round((d.power_status !== undefined ? d.power_status : 0)), endian);
      pos += 1;
    }
