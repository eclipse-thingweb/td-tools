// TS013 Payload Codec — milesight_vs321
// Schema version: 1
// Generated from: vs321.yaml
...
function decodePayload(buf, endian) {
  ...
  endian = endian || "little";
    ...
    while (pos + 2 <= buf.length) {
      ...
      } else if ("[" + channel_id + ", " + channel_type + "]" === "[5, 253]") {
        var people_total_counts = readU(buf, pos, 2, endian);
        pos += 2;
        vars.people_total_counts = people_total_counts;
        d.people_total_counts = people_total_counts;
      } else if ("[" + channel_id + ", " + channel_type + "]" === "[7, 255]") {
        var illuminance_status = readU(buf, pos, 1, endian);
        pos += 1;
        vars.illuminance_status = illuminance_status;
        d.illuminance_status = illuminance_status;
      } else if ("[" + channel_id + ", " + channel_type + "]" === "[8, 244]") {
      ...
      } else if ("[" + channel_id + ", " + channel_type + "]" === "[132, 104]") {
        var humidity = readU(buf, pos, 1, endian);
        pos += 1;
        vars.humidity = humidity;
        d.humidity = humidity;
        var humidity_alarm = readU(buf, pos, 1, endian);
        pos += 1;
        vars.humidity_alarm = humidity_alarm;
        d.humidity_alarm = humidity_alarm;
      } else {
        break;
      }
    }
...
function encodePayload(d, endian) {
  ...
  endian = endian || "little";
    ...
    if (d.people_total_counts !== undefined) {
      writeU(buf, pos, 1, 5, endian);
      pos += 1;
      writeU(buf, pos, 1, 253, endian);
      pos += 1;
      writeU(buf, pos, 2, Math.round((d.people_total_counts !== undefined ? d.people_total_counts : 0)), endian);
      pos += 2;
    }
    if (d.illuminance_status !== undefined) {
      writeU(buf, pos, 1, 7, endian);
      pos += 1;
      writeU(buf, pos, 1, 255, endian);
      pos += 1;
      writeU(buf, pos, 1, Math.round((d.illuminance_status !== undefined ? d.illuminance_status : 0)), endian);
      pos += 1;
    }
    if (d.detection_status !== undefined) {
      writeU(buf, pos, 1, 8, endian);
      pos += 1;
      writeU(buf, pos, 1, 244, endian);
      pos += 1;
    ...
    if (d.humidity !== undefined || d.humidity_alarm !== undefined) {
      ...
      pos += 1;
      writeU(buf, pos, 1, Math.round((d.humidity_alarm !== undefined ? d.humidity_alarm : 0)), endian);
      pos += 1;
    }
